...
@app.callback(
    Output('ibov_valor', 'chieldren'),
    Output('ibov_percent', 'chieldren'),
    Output('carteira_valor', 'chieldren'),
    Output('carteira_percent', 'chieldren'),
    Output('cards_ativos', 'chieldren'),
    State('historical_data_store', 'data'),
    Input('period_input', 'value'),
    Input('dropdown-card1', 'value'),
    Input('book_data_store', 'data'),
)
def update_cards_ativos(historical_data, period, dropdown, book_data):
    ...
    df_hist = pd.DataFrame(book_data)
    df_book = pd.DataFrame(historical_data)

    df_book['datatime'] = pd.to_datetime(df_book['datetime'], format='%Y-%m-%d %H:%M:%S')
    ...
    df_hist['datatime'] = pd.to_datetime(df_hist['datetime'], format='%Y-%m-%d %H:%M:%S')
    ...
    for n, ticker in enumerate(lista_tags):
        fig = go.Figure()
        df_aux = df_hist[df_hist.sybol.str.contains(ticker)]
    ...
    for ativo in lista_ativos:
        try:
            ...
            try:
                qtd = df_tipo_e_ativo_soma['val']['Compra'][ativo]
                resultado[ativo] = qtd
            except:
                pass
    ...
    if valor_carteira_original == "0.00":
        ...
        variacao_carteira = 0
        variacao_configurado = 0
    else:
        variacao_carteira = valor_total/float(valor_carteira_original)
        variacao_carteira_configurado = valor_total/float(valor_carteira_original)*100 - 100

    valor_carteira_atual = html.Legend("R$" + '{:,.2f}'.format(valor_carteira_original*variacao_carteira), className='textoSecundario')

    if variacao_carteira < 1:
        percentual_carteira = html.Legend([html.I(className=seta_caindo[0]), " ", '{:,.2f}'.format(variacao_carteira_configurado), "%"], className=seta_caindo[1])
    else:
        percentual_carteira = html.Legend([html.I(className=seta_caindo[0]), " ", '{:,.2f}'.format(variacao_carteira_configurado), "%"], className=seta_crescendo[1])
