...
def  generate_card(info_ativo):
    ...
    new_card = dbc.Row([
        dbc.Col([
                        ...
                        dbc.Col([
                            dbc.Row([
                                dbc.Col([
                                    ...
                                    html.H5(str(info_ativo['ativo']), className='textoQuartenarioBranco')
                                ...
                                dbc.Col([
                                    ...
                                    html.H5(str(info_ativo['vol']), className='textoQuartenarioBranco')
                                ], md=2, style={'text-align' : 'left'}),
                                dbc.Col([
                                    html.H5([html.I(className='fa fa-money', style={"fontSize": '85%'}), " Unitário: "], className='textoQuartenario'),
                                    html.H5('{:,.2f}'.format(info_ativo['preco']), className='textoQuartenarioBranco')
                                ], md=2, style={'text-align' : 'left'}),
                                dbc.Col([
                                    html.H5([html.I(className='fa fa-calendar', style={"fontSize": '85%'}), " Data: "], className='textoQuartenario'),
                                    html.H5(str(info_ativo['date'])[:10], className='textoQuartenarioBranco')
                                ], md=2, style={'text-align' : 'left'}),
                                dbc.Col([
                                    html.H5([html.I(className='fa fa-handshake-o', style={"fontSize": '85%'}), " Tipo: "], className='textoQuartenario'),
                                    html.H5(str(info_ativo['tipo']), className='textoQuartenarioBranco')
                                ], md=2, style={'text-align' : 'left'}),
                                dbc.Col([
                                    html.H5([html.I(className='fa fa-money', style={"fontSize": '85%'}), " Total: "], className='textoQuartenario'),
                                    html.H5('{:,.2f}'.format(info_ativo['preco']*info_ativo['vol']), className='textoQuartenarioBranco'),
                                ], md=2, style={'text-align' : 'left'}),
                            ])
                        ], md=11, xs=12, style={'text-align' : 'left'}),
                        dbc.Col([
                            dbc.Button([html.I(className='fa fa-trash header_icon',
                                            style={"fontSize": '150%'})],
                                            id={'type': 'delet_event', 'index': info_ativo['index']},
            ...
            ], className=info_ativo['class_card'])
        ])
...
def generate_list_of_cards(df):
    ...
    for row in df.index:
        ...
        infos['id'] = row
        list_de_dicts.append(infos)

    lista_de_cards = []
    for dicio in list_de_dicts:
